...
\author{Gregory Zancewicz\thanks{\url{https://www.linkedin.com/in/zancewicz/}}}
...
This paper provides a self-contained overview of six core model classes arranged in a progression of increasing generality:
...
\begin{enumerate}
  ...
  \item \textbf{Markov Decision Processes} (Section~\ref{sec:mdp}). An agent chooses actions that influence both state transitions and rewards. The Bellman equation characterizes the optimal policy, and solution methods---value iteration, policy iteration, and linear programming---provide constructive algorithms.
  ...
  \item \textbf{Contextual Bandits} (Section~\ref{sec:cb}). Side information (a context vector) is observed before each arm selection, and reward distributions depend on the context. The agent must learn a mapping from contexts to optimal arms. We cover LinUCB, Thompson Sampling for linear models, and EXP4 for adversarial settings.
...
Section~\ref{sec:beyond} briefly surveys several related model classes---hidden Markov models, Markov games, constrained MDPs, semi-Markov decision processes, and model-free reinforcement learning---that lie beyond the scope of this paper but occupy adjacent regions of the same theoretical landscape.
...
A \textbf{policy} $\pi : \mathcal{S} \to \mathcal{A}$ is a rule that specifies which action to take in each state. More generally, a policy may be \emph{stochastic}, specifying a distribution $\pi(a \mid s)$ over actions in each state; however, for finite-state discounted MDPs a deterministic stationary policy always suffices for optimality \cite{puterman1994}. Under a fixed deterministic policy $\pi$, the MDP reduces to an ordinary Markov chain with transition probabilities $p_{ij}(\pi(s_i))$.
...
It is often useful to define the \textbf{action-value function} (or Q-function) $Q^*(s_i, a)$, which gives the value of taking action $a$ in state $s_i$ and then acting optimally:
\begin{equation}
  Q^*(s_i, a) = r(s_i, a) + \gamma \sum_{j=1}^{n} p_{ij}(a)\, V^*(s_j).
\end{equation}
The Bellman equation is then $V^*(s_i) = \max_a Q^*(s_i, a)$. The Q-function is the central object in model-free algorithms such as Q-learning (Section~\ref{sec:beyond}).

\subsection{Solution Methods}

The Bellman equation can be solved by several methods. \textbf{Value iteration} \cite{bellman1957} repeatedly applies the Bellman operator $V_{k+1}(s_i) \leftarrow \max_a [r(s_i, a) + \gamma \sum_j p_{ij}(a)\, V_k(s_j)]$ until convergence; this is the method used in the examples below. \textbf{Policy iteration} \cite{howard1960} alternates between evaluating the current policy (solving a linear system) and improving it greedily with respect to the resulting value function; it converges in a finite number of iterations. Both methods yield the optimal policy; value iteration is simpler to implement, while policy iteration often converges in fewer iterations for large state spaces. The MDP can also be formulated as a \textbf{linear program} \cite{puterman1994}, which provides a third exact solution method.
...
A POMDP is defined by the tuple $(\mathcal{S}, \mathcal{A}, p, r, \Omega, O, \gamma, \mathbf{b}_0)$, where $\mathbf{b}_0$ is the initial belief distribution over $\mathcal{S}$:
...
\end{equation}
and symmetrically for state TR. The listen action leaves the tiger's position unchanged (identity transition: $p_{\text{TL},\text{TL}}(\text{listen}) = p_{\text{TR},\text{TR}}(\text{listen}) = 1$). Opening a door yields $+10$ (treasure) or $-100$ (tiger) and resets the problem with the tiger placed uniformly at random; observations for open actions are deterministic, revealing whether the tiger or treasure was found.
...
The structure of the Tiger Problem is shown in Figure~\ref{fig:tiger}.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[->, >=Stealth, shorten >=1pt, auto,
                      ...
                      action/.style={draw, dotted, rectangle, rounded corners=3pt,
                                     minimum size=0.7cm, inner sep=2pt, fill=white},
                      obs/.style={draw, dashed, diamond, inner sep=1pt, fill=yellow!10,
                                  minimum size=0.6cm}]

    % States
    \node[state] (TL) {TL};
    \node[state] (TR) [right=6cm of TL] {TR};

    % Listen action node (centered)
    \node[action] (listen) [below=2cm of $(TL)!0.5!(TR)$] {listen};

    % Observation nodes
    \node[obs] (HL) [below left=1.5cm and 0.8cm of listen] {\small HL};
    \node[obs] (HR) [below right=1.5cm and 0.8cm of listen] {\small HR};

    % Open actions
    \node[action] (OL) [above left=0.5cm and 2cm of TL] {open-L};
    \node[action] (OR) [above right=0.5cm and 2cm of TR] {open-R};

    % Edges: states to listen
    \path (TL) edge (listen)
          (TR) edge (listen);

    % Edges: listen to observations
    \path (listen) edge node[left] {$0.85$} (HL)
          (listen) edge node[right] {$0.15$} (HR);
    \node[above left=0.05cm and 0.3cm of HR, font=\scriptsize\itshape, text=gray] {(if TL)};

    % Open-left outcomes
    \path (OL) edge [bend right=10] node[left, font=\small\itshape] {$-100$} (TL)
          (OL) edge [bend left=30] node[above, font=\small\itshape] {$+10$} (TR);

    % Open-right outcomes
    \path (OR) edge [bend right=30] node[above, font=\small\itshape] {$+10$} (TL)
          (OR) edge [bend left=10] node[right, font=\small\itshape] {$-100$} (TR);

    % Listen reward
    \node[below=0.1cm of listen, font=\small\itshape] {$r = -1$};

  \end{tikzpicture}
  \caption{The Tiger Problem. States TL (tiger left) and TR (tiger right) are hidden. The listen action yields a noisy observation (dashed diamonds): hearing HL with probability $0.85$ when the tiger is left. Opening a door yields $+10$ (treasure) or $-100$ (tiger) and resets the problem.}
  \label{fig:tiger}
\end{figure}
...
In the frequentist formulation, the MAB can be viewed as a degenerate MDP with a single observable state: the only decision is which action (arm) to take, and there are no state transitions. In the Bayesian formulation, however, the agent's posterior beliefs over arm parameters evolve with each observation, making the problem an MDP over belief states---and it is this richer formulation that gives rise to the Gittins index. The MAB can also be viewed as a POMDP in which the hidden state is the vector of true arm means and each reward observation provides partial information about them; the Bayesian perspective connects directly to the belief-state framework of Section~\ref{sec:pomdp}.

The problem was first studied by Thompson \cite{thompson1933} and formalized in its modern form by Robbins \cite{robbins1952}. Lattimore and Szepesv\'ari \cite{lattimore2020} provide a comprehensive modern treatment.
...
\textbf{Remark (frequentist vs.\ Bayesian regret).} The definition above is \emph{frequentist} (or worst-case) regret: the arm means $\mu_1, \dots, \mu_K$ are fixed unknowns, and the expectation is over the algorithm's randomness. In the Bayesian formulation, the arm means are drawn from a prior distribution and \emph{Bayesian regret} averages over both the prior and the algorithm: $\mathbb{E}_{\boldsymbol{\mu}}[\mathbb{E}[R_T \mid \boldsymbol{\mu}]]$. The Lai--Robbins lower bound and the UCB1 guarantee stated below are frequentist results; the Gittins index is optimal in the Bayesian sense; Thompson Sampling achieves strong guarantees in both frameworks.

Figure~\ref{fig:mab} illustrates the structure.

\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[>=Stealth, thick,
                      arm/.style={draw, rectangle, rounded corners=5pt, minimum width=1.2cm,
                                  minimum height=2.2cm, fill=gray!10, font=\small},
                      reward/.style={draw, circle, fill=yellow!15, minimum size=0.7cm,
                                     font=\small}]

    % Agent
    \node[draw, ellipse, fill=blue!10, minimum width=2cm, minimum height=1cm,
          font=\small\bfseries] (agent) at (0, 0) {Agent};

    % Arms
    \node[arm] (a1) at (-4, -3.5) {Arm 1};
    \node[arm] (a2) at (-1.3, -3.5) {Arm 2};
    \node[arm] (a3) at (1.3, -3.5) {Arm 3};
    \node[font=\Large] at (3, -3.5) {$\cdots$};
    \node[arm] (aK) at (4.5, -3.5) {Arm $K$};

    % Unknown means
    \node[below=0.1cm of a1, font=\scriptsize\itshape] {$\mu_1 = {?}$};
    \node[below=0.1cm of a2, font=\scriptsize\itshape] {$\mu_2 = {?}$};
    \node[below=0.1cm of a3, font=\scriptsize\itshape] {$\mu_3 = {?}$};
    \node[below=0.1cm of aK, font=\scriptsize\itshape] {$\mu_K = {?}$};

    % Pull arrows (agent to arms)
    \draw[->, dashed] (agent) -- (a1) node[midway, left, font=\scriptsize] {pull};
    \draw[->, dashed] (agent) -- (a2);
    \draw[->, dashed] (agent) -- (a3);
    \draw[->, dashed] (agent) -- (aK);

    % Reward arrow (one arm back to agent, highlighted)
    \draw[->, thick, densely dotted] (a2.north) to[bend left=20]
          node[midway, right, font=\scriptsize] {$X_{I_t,t}$} (agent.south west);

    % Selection indicator
    \draw[thick, red!70!black] ([xshift=-0.3cm, yshift=-0.1cm]a2.north west)
          rectangle ([xshift=0.3cm, yshift=0.1cm]a2.north east);
    \node[above=0.3cm of a2, font=\scriptsize, red!70!black] {$I_t = 2$};

  \end{tikzpicture}
  \caption{The multi-armed bandit. An agent selects one of $K$ arms at each round. The chosen arm (highlighted) returns a stochastic reward $X_{I_t,t}$; the true means $\mu_i$ are unknown and must be learned through interaction.}
  \label{fig:mab}
\end{figure}
...
The simplest bandit strategy is $\varepsilon$-greedy: with probability $1 - \varepsilon$, pull the arm with the highest empirical mean (exploit); with probability $\varepsilon$, pull an arm uniformly at random (explore). While intuitive, a fixed $\varepsilon$ yields linear regret because the agent continues exploring at rate $\varepsilon$ indefinitely, even after the best arm is identified. Decaying $\varepsilon_t \propto t^{-1/3}$ improves this to $O(T^{2/3})$ \cite{auer2002}, but this is still far worse than the $O(\ln T)$ achievable by the methods below.
...
For the discounted infinite-horizon Bayesian bandit with independent, rested arms (discount factor $\gamma$, known prior on each arm), Gittins \cite{gittins1979} proved a remarkable result: the optimal policy is an \emph{index policy}. Each arm~$i$ is assigned a \textbf{Gittins index} $G_i$ computed from its posterior alone, and the agent always pulls the arm with the highest index. The index can be interpreted as the fair price at which the agent would be indifferent between continuing to pull the arm and receiving a guaranteed constant payoff forever. The Gittins index theorem shows that the multi-arm problem decomposes into independent single-arm problems---a dramatic simplification. This optimality holds specifically for the discounted infinite-horizon objective with independent, rested arms and a known prior; it breaks down under finite horizons, average-reward criteria, or when arms evolve even while not being pulled---motivating the restless bandit model of Section~\ref{sec:restless}.
...
\end{equation}
which is $O(K \ln T)$---logarithmic in $T$ and matching the Lai--Robbins lower bound up to constant factors for bounded rewards. Thompson Sampling also achieves asymptotically optimal regret under standard regularity conditions on the prior \cite{agrawal2012}, with expected regret
\begin{equation}
  \mathbb{E}[R_T] = O\left(\sum_{i:\, \mu_i < \mu^*} \frac{\Delta_i \ln T}{\text{KL}(\nu_i, \nu^*)}\right),
\end{equation}
matching the Lai--Robbins lower bound up to constant factors. Figure~\ref{fig:regret} illustrates the qualitative separation between these growth rates.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[>=Stealth, thick, scale=0.9]
    ...
    \draw[->] (0,0) -- (0,6.5) node[above] {$\mathbb{E}[R_T]$};

    % Linear regret: epsilon-greedy (fixed)
    \draw[thick, red!70!black, domain=0:10, samples=50]
      plot (\x, {0.5*\x}) node[right, font=\small] {$O(T)$};
    \node[red!70!black, font=\scriptsize, right] at (10, 5.5) {fixed $\varepsilon$-greedy};

    % Sublinear regret: decaying epsilon-greedy O(T^{2/3})
    \draw[thick, orange, domain=0.1:10, samples=80]
      plot (\x, {0.8*\x^(2/3)}) node[right, font=\small] {};
    \node[orange, font=\scriptsize, right] at (10, 3.7) {$O(T^{2/3})$};
    \node[orange, font=\scriptsize, right] at (10, 3.2) {decaying $\varepsilon$};

    % Logarithmic regret: UCB1/Thompson/KL-UCB
    \draw[thick, blue!70!black, domain=0.5:10, samples=80]
      plot (\x, {1.2*ln(\x)/ln(2)}) node[right, font=\small] {};
    \node[blue!70!black, font=\scriptsize, right] at (10, 2.1) {$O(\ln T)$};
    \node[blue!70!black, font=\scriptsize, right] at (10, 1.6) {UCB1 / TS / KL-UCB};

    % Sqrt regret: EXP3 (adversarial)
    \draw[thick, green!50!black, dashed, domain=0.1:10, samples=80]
      plot (\x, {1.0*sqrt(\x)}) node[right, font=\small] {};
    \node[green!50!black, font=\scriptsize, right] at (10, 2.8) {$O(\!\sqrt{T})$};
    \node[green!50!black, font=\scriptsize, right] at (10, 2.35) {EXP3 (adversarial)};

    % Lai-Robbins lower bound
    \draw[thick, black, densely dotted, domain=0.5:10, samples=80]
      plot (\x, {0.9*ln(\x)/ln(2)});
    \node[black, font=\scriptsize, right] at (7.2, 0.6) {Lai--Robbins};
    \node[black, font=\scriptsize, right] at (7.2, 0.15) {lower bound};

  \end{tikzpicture}
  \caption{Qualitative regret growth rates for bandit strategies (not to scale). The Lai--Robbins lower bound (dotted) shows that $O(\ln T)$ is the best achievable rate for stochastic bandits. Fixed $\varepsilon$-greedy incurs linear regret; decaying $\varepsilon$ achieves $O(T^{2/3})$; UCB1, Thompson Sampling, and KL-UCB achieve the optimal $O(\ln T)$ rate. EXP3 achieves $O(\sqrt{T})$ in the harder adversarial setting.}
  \label{fig:regret}
\end{figure}
...
\section{Contextual Bandit Problems}
\label{sec:cb}

The \textbf{contextual bandit} problem extends the multi-armed bandit framework of Section~\ref{sec:mab} by incorporating \emph{side information}---a context vector observed before each arm selection. The context may encode user features in a recommender system, patient characteristics in personalized medicine, or the current state of a bidding environment in online advertising. Unlike a full MDP, the context arrives exogenously at each round and does not depend on past actions; the agent observes a context, selects an arm, and receives a reward, but there are no state transitions. Contextual bandits thus occupy a middle ground between the stateless exploration of Section~\ref{sec:mab} and the full sequential planning of Section~\ref{sec:mdp}.

The contextual bandit model was formalized by Langford and Zhang \cite{langford2007}, who introduced the Epoch-Greedy algorithm, and has since become a cornerstone of online learning in applications ranging from news article recommendation to clinical trial design. A particularly influential algorithm is LinUCB \cite{li2010}, which applies the principle of optimism in the face of uncertainty to a linear reward model.

The key algorithmic challenge is the same as in standard bandits---balancing exploration and exploitation---but now the agent must generalize across contexts. A policy that performs well in one context may be suboptimal in another, and the agent must learn a mapping from contexts to arm selections.
...
\begin{tabular}{ll}
  \textbf{Symbol} & \textbf{Description} \\
  \hline
  $\mathcal{X} \subseteq \mathbb{R}^d$ & Context space \\
  $x_t$ & Context vector observed at round $t$ \\
  $K$ & Number of arms (actions) \\
  $\boldsymbol{\phi}(x, a)$ & Feature vector for context--arm pair $(x, a)$ \\
  $d$ & Dimension of feature vectors \\
  $\boldsymbol{\theta}^*$ & Unknown parameter vector \\
  $r_t(a)$ & Reward for arm $a$ at round $t$ \\
  $\eta_t$ & Zero-mean noise in reward model \\
  $\pi: \mathcal{X} \to \mathcal{A}$ & Policy mapping contexts to arms \\
  $R_T$ & Cumulative regret over $T$ rounds \\
  $\mathbf{V}_t$ & Design matrix (covariance estimate) at round $t$ \\
  $\mathbf{b}_t$ & Cumulative reward-weighted features at round $t$\footnotemark \\
  $\hat{\boldsymbol{\theta}}_t$ & Least-squares estimate of $\boldsymbol{\theta}^*$ at round $t$ \\
  $\alpha$ & Exploration parameter (confidence width)\footnotemark \\
  $\lambda$ & Regularization parameter\footnotemark \\
  $\sigma^2$ & Noise variance (Thompson Sampling) \\
  $\sigma_\theta^2$ & Prior variance for $\boldsymbol{\theta}$ \\
  $\boldsymbol{\mu}_t, \boldsymbol{\Sigma}_t$ & Posterior mean and covariance of $\boldsymbol{\theta}$\footnotemark \\
  $\tilde{\boldsymbol{\theta}}$ & Posterior sample (Thompson Sampling) \\
  $N$ & Number of experts (EXP4) \\
  $w_{i,t}$ & Weight of expert $i$ at round $t$ (EXP4) \\
  $p_t(a)$ & Mixing probability over arms at round $t$ (EXP4) \\
  $\hat{r}_t(a)$ & Importance-weighted reward estimate (EXP4) \\
  $\eta$ & Learning rate (EXP4)\footnotemark \\
\end{tabular}

\addtocounter{footnote}{-4}
\stepcounter{footnote}\footnotetext{Not to be confused with the belief state $\mathbf{b}$ of Section~\ref{sec:pomdp} or the immediate return $b$ in Bellman's notation (Section~\ref{sec:mdp}).}
\stepcounter{footnote}\footnotetext{This $\alpha$ is unrelated to the transition probability $\alpha$ in Section~\ref{sec:mc} or the Beta shape parameters in Section~\ref{sec:mab}.}
\stepcounter{footnote}\footnotetext{This $\lambda$ is unrelated to the Lagrange multiplier $\lambda$ in Section~\ref{sec:restless}.}
\stepcounter{footnote}\footnotetext{Not to be confused with the state distribution $\boldsymbol{\mu}_t$ in Section~\ref{sec:mc}.}
\stepcounter{footnote}\footnotetext{The EXP4 learning rate $\eta$ is unrelated to the noise variable $\eta_t$ in the linear reward model above.}

\subsection{Formal Definition}

The contextual bandit proceeds in rounds $t = 1, 2, \dots, T$. At each round:
\begin{enumerate}
  \item Nature reveals a context $x_t \in \mathcal{X}$.
  \item The agent selects an arm $a_t \in \{1, \dots, K\}$, possibly based on the context and past observations.
  \item The agent receives reward $r_t(a_t)$ and observes no reward for the other arms.
\end{enumerate}

In the \textbf{stochastic linear model}, the expected reward for arm~$a$ given context~$x$ is
\begin{equation}
  \mathbb{E}[r_t(a) \mid x_t] = \boldsymbol{\phi}(x_t, a)^\top \boldsymbol{\theta}^*,
\end{equation}
where $\boldsymbol{\phi}: \mathcal{X} \times \{1, \dots, K\} \to \mathbb{R}^d$ is a known feature map and $\boldsymbol{\theta}^* \in \mathbb{R}^d$ is an unknown parameter vector. The actual reward is often modeled as
\begin{equation}
  r_t(a) = \boldsymbol{\phi}(x_t, a)^\top \boldsymbol{\theta}^* + \eta_t,
\end{equation}
where $\eta_t$ is zero-mean noise (e.g., sub-Gaussian).

The agent's goal is to minimize \textbf{cumulative regret}, defined as
\begin{equation}
  R_T = \sum_{t=1}^{T} \left[ \max_{a \in \{1,\dots,K\}} \boldsymbol{\phi}(x_t, a)^\top \boldsymbol{\theta}^* - \boldsymbol{\phi}(x_t, a_t)^\top \boldsymbol{\theta}^* \right],
\end{equation}
the total difference between the reward of the optimal arm in each context and the arm actually chosen. Unlike the standard bandit setting, the optimal arm $a^*(x_t) = \arg\max_a \boldsymbol{\phi}(x_t, a)^\top \boldsymbol{\theta}^*$ may change from round to round depending on the context.

Two common settings for the context arrival process are:
\begin{itemize}
  \item \textbf{Stochastic contexts}: $x_1, x_2, \dots$ are drawn i.i.d.\ from some fixed distribution over $\mathcal{X}$.
  \item \textbf{Adversarial contexts}: $x_1, x_2, \dots$ are chosen by an adversary who may observe the agent's past behavior but not its future randomization.
\end{itemize}

\subsection{LinUCB: Linear Upper Confidence Bound}

LinUCB \cite{li2010} is the most widely-used contextual bandit algorithm in practice. It extends the optimistic principle behind UCB1 to linear reward models, maintaining uncertainty estimates via ridge regression. Where UCB1 estimates a fixed unknown mean for each arm independently, LinUCB models each arm's reward as a linear function of the observed context, enabling generalization across similar contexts.

At each round, LinUCB maintains a ridge-regression estimate of $\boldsymbol{\theta}^*$. Let $\mathbf{V}_t \in \mathbb{R}^{d \times d}$ denote the design matrix and $\mathbf{b}_t \in \mathbb{R}^d$ the cumulative reward-weighted features after $t$ rounds:
\begin{equation}
  \mathbf{V}_t = \lambda \mathbf{I} + \sum_{\tau=1}^{t-1} \boldsymbol{\phi}(x_\tau, a_\tau) \boldsymbol{\phi}(x_\tau, a_\tau)^\top, \quad
  \mathbf{b}_t = \sum_{\tau=1}^{t-1} r_\tau \boldsymbol{\phi}(x_\tau, a_\tau),
\end{equation}
where $\lambda > 0$ is a regularization parameter. The ridge estimate is $\hat{\boldsymbol{\theta}}_t = \mathbf{V}_t^{-1} \mathbf{b}_t$.

The upper confidence bound for arm $a$ at round $t$ is
\begin{equation}
  \text{UCB}_t(a) = \boldsymbol{\phi}(x_t, a)^\top \hat{\boldsymbol{\theta}}_t + \alpha \sqrt{\boldsymbol{\phi}(x_t, a)^\top \mathbf{V}_t^{-1} \boldsymbol{\phi}(x_t, a)},
\end{equation}
where $\alpha > 0$ is an exploration parameter. The first term is the predicted reward; the second is the \emph{confidence width}, equal to the Mahalanobis norm of $\boldsymbol{\phi}(x_t, a)$ under $\mathbf{V}_t^{-1}$. Contexts whose feature vectors lie in under-explored directions receive larger bonuses.

The agent selects $a_t = \arg\max_{a} \text{UCB}_t(a)$, observes reward $r_t$, and updates $\mathbf{V}_{t+1}$ and $\mathbf{b}_{t+1}$.

\textbf{Regret bound.} Under standard assumptions (bounded features, sub-Gaussian noise), LinUCB achieves
\begin{equation}
  \mathbb{E}[R_T] = O(d \sqrt{T \log T}).
\end{equation}
This bound is optimal up to logarithmic factors for $d$-dimensional linear contextual bandits. The key advantage over non-contextual UCB1 is that regret scales with feature dimension $d$ rather than arm count $K$---when $K \gg d$, this is a dramatic improvement.

\subsection{Thompson Sampling for Contextual Bandits}

Thompson Sampling extends naturally from the multi-armed bandit setting of Section~\ref{sec:mab} to the contextual case. Rather than maintaining independent Beta posteriors over each arm's success probability, the agent maintains a posterior distribution over $\boldsymbol{\theta}$ and samples from it at each round.

For linear models, assume:
\begin{equation}
  r_t = \boldsymbol{\phi}(x_t, a_t)^\top \boldsymbol{\theta} + \eta_t, \quad \eta_t \sim \mathcal{N}(0, \sigma^2), \quad \boldsymbol{\theta} \sim \mathcal{N}(\mathbf{0}, \sigma_\theta^2 \mathbf{I}).
\end{equation}
The posterior over $\boldsymbol{\theta}$ remains Gaussian by conjugacy:
\begin{equation}
  \boldsymbol{\theta} \mid \text{data} \sim \mathcal{N}(\boldsymbol{\mu}_t, \boldsymbol{\Sigma}_t),
\end{equation}
where
\begin{align}
  \boldsymbol{\Sigma}_t &= \left( \sigma^{-2} \sum_{s=1}^{t} \boldsymbol{\phi}(x_s, a_s) \boldsymbol{\phi}(x_s, a_s)^\top + \sigma_\theta^{-2} \mathbf{I} \right)^{-1}, \\
  \boldsymbol{\mu}_t &= \sigma^{-2} \boldsymbol{\Sigma}_t \sum_{s=1}^{t} r_s \boldsymbol{\phi}(x_s, a_s).
\end{align}

At each round: (1) observe $x_t$, (2) sample $\tilde{\boldsymbol{\theta}} \sim \mathcal{N}(\boldsymbol{\mu}_{t-1}, \boldsymbol{\Sigma}_{t-1})$, (3) select $a_t = \arg\max_a \boldsymbol{\phi}(x_t, a)^\top \tilde{\boldsymbol{\theta}}$, and (4) update the posterior.

\textbf{Regret bound.} Thompson Sampling for linear contextual bandits achieves $\mathbb{E}[R_T] = O(d \sqrt{T \log T})$ \cite{agrawal2013}, matching LinUCB. It often performs better empirically because exploration is proportional to uncertainty in a natural Bayesian sense, and it requires no tuning of a confidence parameter $\alpha$.

\subsection{EXP4: Adversarial Contextual Bandits}

The stochastic setting assumes rewards are drawn from fixed distributions. In adversarial settings, an adversary may choose rewards after observing the algorithm's behavior. \textbf{EXP4} \cite{auer2002nonstochastic} addresses this by competing with a finite set of $N$ \emph{experts} (policies) $\pi_1, \dots, \pi_N$, where each $\pi_i : \mathcal{X} \to \mathcal{A}$.

EXP4 maintains weights $w_{i,t}$ over experts. At each round:
\begin{enumerate}
  \item Compute mixing probabilities: $p_t(a) = \sum_i w_{i,t} \mathbb{I}[\pi_i(x_t) = a] / \sum_j w_{j,t}$.
  \item Sample $a_t \sim p_t(\cdot)$ and observe $r_t(a_t)$.
  \item Construct importance-weighted estimates: $\hat{r}_t(a) = r_t(a)/p_t(a)$ if $a = a_t$, else $0$.
  \item Update weights: $w_{i,t+1} = w_{i,t} \exp(\eta \hat{r}_t(\pi_i(x_t)))$.
\end{enumerate}

\textbf{Regret bound.} EXP4 achieves $\mathbb{E}[R_T] = O(\sqrt{TK \ln N})$ against the best expert in hindsight, with no distributional assumptions. This is the price of adversarial robustness: the bound depends on $\sqrt{K}$ rather than $\sqrt{d}$.

\subsection{Example: News Article Recommendation}

Consider a news website recommending articles to users. There are two user types (contexts): ``tech-interested'' and ``sports-interested,'' and three articles (arms): Tech, Sports, and General. We use a 2-dimensional feature representation for tractable hand calculation.

\textbf{Feature vectors:}
\begin{center}
\begin{tabular}{lcc}
  Context--Arm Pair & $\boldsymbol{\phi}$ & Expected Reward \\
  \hline
  Tech user, Tech article & $[1, 0]$ & $0.8$ \\
  Tech user, Sports article & $[0, 0.5]$ & $0.2$ \\
  Tech user, General article & $[0.5, 0.5]$ & $0.6$ \\
  Sports user, Tech article & $[0, 0.5]$ & $0.2$ \\
  Sports user, Sports article & $[1, 0]$ & $0.8$ \\
  Sports user, General article & $[0.5, 0.5]$ & $0.6$ \\
\end{tabular}
\end{center}

The true parameter is $\boldsymbol{\theta}^* = [0.8, 0.4]$, so expected rewards are $\boldsymbol{\phi}^\top \boldsymbol{\theta}^*$. Tech users prefer Tech articles (0.8 vs 0.6 vs 0.2); Sports users prefer Sports articles (0.8 vs 0.6 vs 0.2).

\textbf{LinUCB trace} with $\alpha = 1$, $\lambda = 1$, alternating contexts, and noiseless rewards ($r_t = \boldsymbol{\phi}^\top \boldsymbol{\theta}^*$):

\begin{center}
\begin{tabular}{ccccccc}
  $t$ & Context & $\hat{\boldsymbol{\theta}}_t$ & UCB(Tech) & UCB(Sports) & UCB(Gen) & Choice \\
  \hline
  1 & Sports & $[0.00,\; 0.00]$ & 0.50 & 1.00 & 0.71 & Sports \\
  2 & Tech & $[0.40,\; 0.00]$ & 1.11 & 0.50 & 0.81 & Tech \\
  3 & Sports & $[0.53,\; 0.00]$ & 0.50 & 1.11 & 0.85 & Sports \\
  4 & Tech & $[0.60,\; 0.00]$ & 1.10 & 0.50 & 0.86 & Tech \\
  5 & Sports & $[0.64,\; 0.00]$ & 0.50 & 1.09 & 0.87 & Sports \\
  6 & Tech & $[0.67,\; 0.00]$ & 1.08 & 0.50 & 0.87 & Tech \\
\end{tabular}
\end{center}

The feature structure enables LinUCB to identify the correct arm for each user type from the first round: the matching arm (Sports for Sports users, Tech for Tech users) has feature vector $[1, 0]$, which has the largest norm and thus the highest exploration bonus. As data accumulates, the first component of $\hat{\boldsymbol{\theta}}_t$ converges toward $0.8$, and the algorithm consistently matches users with their preferred content. Note that $\hat{\boldsymbol{\theta}}_t$ converges to approximately $[0.8, 0]$ rather than the true $[0.8, 0.4]$: because the algorithm always selects arms whose features lie along $[1, 0]$, the second feature dimension is never explored in these early rounds. This illustrates a known property of linear bandits---the algorithm may not recover the full parameter vector, yet it still selects the correct arms. (After many more rounds, the persistent uncertainty in the second dimension eventually causes the General article's UCB to exceed the matching arm's, triggering exploration of the remaining parameter.)

The following Python code verifies this solution:

\begin{lstlisting}[caption={LinUCB for the news recommendation example.}]
import numpy as np

d, K = 2, 3
alpha, lambda_reg = 1.0, 1.0
theta_star = np.array([0.8, 0.4])

def get_features(user, arm):
    feats = {(0,0): [1,0], (0,1): [0,0.5], (0,2): [0.5,0.5],
             (1,0): [0,0.5], (1,1): [1,0], (1,2): [0.5,0.5]}
    return np.array(feats[(user, arm)])

V = lambda_reg * np.eye(d)
b = np.zeros(d)
contexts = [1, 0, 1, 0, 1, 0]  # Sports, Tech, alternating

# Trace: 6 rounds with noiseless rewards (reproduces table)
for t in range(6):
    user = contexts[t]
    theta_hat = np.linalg.solve(V, b)
    ucbs = [get_features(user, a) @ theta_hat +
            alpha * np.sqrt(get_features(user, a) @
            np.linalg.solve(V, get_features(user, a))) for a in range(K)]
    arm = np.argmax(ucbs)
    phi = get_features(user, arm)
    r = phi @ theta_star  # noiseless for trace verification
    V += np.outer(phi, phi)
    b += r * phi

print(f"Estimated theta: {np.linalg.solve(V, b)}")  # [0.80, 0.00]
\end{lstlisting}

\subsection{Summary}

Contextual bandits extend the multi-armed bandit by incorporating side information that changes the optimal arm from round to round. The agent must learn a mapping from contexts to actions, generalizing across similar contexts rather than treating each arm independently. LinUCB and Thompson Sampling achieve $O(d\sqrt{T})$ regret under linear reward models, with regret scaling in feature dimension rather than arm count. EXP4 provides adversarial robustness at the cost of worse dependence on the action space. These algorithms underpin modern recommendation systems, personalized medicine, and online advertising.

\subsubsection{Key Equations}

Expected reward (linear model):
\begin{equation*}
  \mathbb{E}[r_t(a) \mid x_t] = \boldsymbol{\phi}(x_t, a)^\top \boldsymbol{\theta}^*.
\end{equation*}
...
Cumulative regret:
\begin{equation*}
  R_T = \sum_{t=1}^{T} \left[ \max_{a} \boldsymbol{\phi}(x_t, a)^\top \boldsymbol{\theta}^* - \boldsymbol{\phi}(x_t, a_t)^\top \boldsymbol{\theta}^* \right].
\end{equation*}
...
LinUCB / Thompson Sampling regret:
\begin{equation*}
  \mathbb{E}[R_T] = O(d \sqrt{T \log T}).
\end{equation*}
...
The joint state space of a $K$-arm restless bandit is the product $\mathcal{S}_1 \times \cdots \times \mathcal{S}_K$, which grows exponentially in $K$. Papadimitriou and Tsitsiklis \cite{papadimitriou1999} proved that the restless bandit problem is \textbf{PSPACE-hard}---a strictly stronger form of intractability than NP-hardness. This motivates the use of index heuristics.
...
\end{equation}
where rows correspond to the current state (G, B) and columns to the next state. Under passive operation, a Good machine degrades with probability $0.1$ per period; a Bad machine self-repairs with probability $0.2$. Active maintenance (the repairman) reduces degradation and greatly improves repair: a Bad machine returns to Good with probability $0.7$ instead of $0.2$. Figure~\ref{fig:restless-machine} shows these transitions.

\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[->, >=Stealth, shorten >=1pt, auto, thick,
                      every state/.style={fill=gray!10, minimum size=1.2cm}]

    % --- Passive (left) ---
    \node[font=\bfseries] at (2, 3) {Passive ($a = 0$)};
    \node[state] (G0) at (0, 0) {G};
    \node[state] (B0) at (4, 0) {B};

    \path (G0) edge [loop above] node {$0.9$} (G0)
          (G0) edge [bend left=20] node[above] {$0.1$} (B0)
          (B0) edge [bend left=20] node[below] {$0.2$} (G0)
          (B0) edge [loop above] node {$0.8$} (B0);

    \node[below=0.3cm of G0, font=\small\itshape] {$r = 1$};
    \node[below=0.3cm of B0, font=\small\itshape] {$r = 0$};

    % --- Active (right) ---
    \node[font=\bfseries] at (9.5, 3) {Active ($a = 1$)};
    \node[state] (G1) at (7.5, 0) {G};
    \node[state] (B1) at (11.5, 0) {B};

    \path (G1) edge [loop above] node {$0.95$} (G1)
          (G1) edge [bend left=20] node[above] {$0.05$} (B1)
          (B1) edge [bend left=20] node[below] {$0.7$} (G1)
          (B1) edge [loop above] node {$0.3$} (B1);

    \node[below=0.3cm of G1, font=\small\itshape] {$r = 1$};
    \node[below=0.3cm of B1, font=\small\itshape] {$r = 0$};

    % Separator
    \draw[dashed, gray] (5.75, -1.5) -- (5.75, 3.5);

  \end{tikzpicture}
  \caption{Single-arm transitions for the machine maintenance restless bandit. Left: passive (unattended) transitions. Right: active (repairman assigned) transitions. The key difference is in the Bad$\to$Good probability ($0.2$ vs.\ $0.7$). Unlike classical bandits, the passive transitions are \emph{not} frozen---the machine degrades whether or not the repairman is present.}
  \label{fig:restless-machine}
\end{figure}
...
The Whittle index policy assigns the repairman to the machine with the highest index---always a Bad machine when one exists. We trace five steps starting from states $(\text{G}, \text{B}, \text{B})$, where M1, M2, M3 denote machines~1, 2, and~3 respectively:
...
The preceding sections trace a single thread through the landscape of Markov models: passive chains (Section~\ref{sec:mc}), controlled chains with full observability (Section~\ref{sec:mdp}), controlled chains with partial observability (Section~\ref{sec:pomdp}), stateless exploration--exploitation (Section~\ref{sec:mab}), exploration with side information (Section~\ref{sec:cb}), and the restless generalization where arms evolve independently (Section~\ref{sec:restless}). This thread is coherent but not exhaustive. Several major model classes occupy adjacent or intersecting regions of the same landscape and are briefly noted here.

\subsection{Hidden Markov Models}

A \textbf{hidden Markov model} (HMM) \cite{rabiner1989} is a Markov chain whose states are unobservable; at each time step the system emits an observation drawn from a state-dependent distribution. The HMM is the passive, uncontrolled counterpart of the POMDP: it shares the hidden-state and observation machinery of Section~\ref{sec:pomdp} but has no actions or rewards. This completes a natural $2 \times 2$ classification---\{observable, hidden\} $\times$ \{passive, controlled\} = \{Markov chain, HMM, MDP, POMDP\}. The central computational problems for HMMs---filtering (the forward algorithm), smoothing (the forward--backward algorithm), and parameter estimation (Baum--Welch / EM)---underpin speech recognition, gene finding, and financial time-series analysis.
...
The worked examples throughout the paper---the two-state Markov chain, the binary MDP and asset maintenance problem, the Tiger Problem, the three-armed bandit under four strategies, the news recommendation contextual bandit, and the machine maintenance restless bandit---are intended to make the formal definitions concrete. Each example is accompanied by explicit numerical computation and, where appropriate, Python code for verification. Our hope is that these examples lower the barrier to entry for practitioners encountering these models for the first time.

Section~\ref{sec:beyond} surveyed several important model classes that lie outside the scope of this paper, including hidden Markov models, Markov games, constrained MDPs, semi-Markov decision processes, and model-free reinforcement learning. Together with the models treated here, they span much of the landscape of Markov-based sequential decision-making. Exploring these extensions---and the connections among them---remains a rich area of both theoretical research and practical application. These models collectively form the mathematical foundation for modern applications ranging from autonomous systems and healthcare decision-making to recommendation engines and game-playing artificial intelligence.
...
\item[Contextual bandit] A bandit model in which the agent observes a context (feature vector) before selecting an arm, and the reward distribution depends on the context. The agent must learn a mapping from contexts to optimal arms. Intermediate between the context-free bandit and the full MDP. (Section~\ref{sec:cb})
...
\item[LinUCB] Linear Upper Confidence Bound. A contextual bandit algorithm that models expected rewards as linear in context features and uses ridge regression to estimate parameters with confidence bounds. Achieves $O(d\sqrt{T})$ regret where $d$ is the feature dimension. (Section~\ref{sec:cb})
...
\bibitem{bellman1957}
  ...
  \emph{Journal of Mathematics and Mechanics},
  vol.~6, no.~5, pp.~679--684, 1957.
...
\bibitem{gittins1979}
  ...
  vol.~41, no.~2, pp.~148--164 [with discussion, pp.~165--177], 1979.
...
\bibitem{agrawal2012}
  ...
  in \emph{Proceedings of the 25th Annual Conference on Learning Theory (COLT)},
  pp.~39.1--39.26, 2012.
...
\bibitem{langford2007}
  J.~Langford and T.~Zhang,
  ``The Epoch-Greedy algorithm for contextual multi-armed bandits,''
  in \emph{Advances in Neural Information Processing Systems~20 (NeurIPS)},
  pp.~817--824, 2007.

\bibitem{li2010}
  L.~Li, W.~Chu, J.~Langford, and R.~E.~Schapire,
  ``A contextual-bandit approach to personalized news article recommendation,''
  in \emph{Proceedings of the 19th International Conference on World Wide Web (WWW)},
  pp.~661--670, 2010.

\bibitem{agrawal2013}
  S.~Agrawal and N.~Goyal,
  ``Thompson Sampling for contextual bandits with linear payoffs,''
  in \emph{Proceedings of the 30th International Conference on Machine Learning (ICML)},
  pp.~127--135, 2013.
