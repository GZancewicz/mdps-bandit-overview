...
\end{equation}
where $\mu^* = \max_i \mu_i$ is the mean of the best arm, $\Delta_i = \mu^* - \mu_i$ is the \textbf{gap} (suboptimality) of arm~$i$, and $T_i(T)$ is the number of times arm~$i$ is pulled in $T$ rounds. Regret measures how much reward is lost by not always pulling the best arm. Figure~\ref{fig:mab} illustrates the structure.
...
\begin{tabular}{ll}
  ...
  $\alpha$ & Exploration parameter (confidence width) \\
...
LinUCB \cite{li2010} is the most widely-used contextual bandit algorithm in practice. It extends the optimistic principle behind UCB1 to linear reward models, maintaining uncertainty estimates via ridge regression. Where UCB1 assigns a fixed unknown parameter to each arm, LinUCB models each arm's reward as a linear function of the observed context, enabling generalization across similar contexts.
...
\textbf{LinUCB trace} with $\alpha = 1$, $\lambda = 1$:

\begin{center}
\begin{tabular}{ccccccc}
  $t$ & Context & $\hat{\boldsymbol{\theta}}_t$ & UCB(Tech) & UCB(Sports) & UCB(Gen) & Choice \\
  \hline
  1 & Sports & $[0, 0]$ & 1.41 & 1.41 & 1.58 & General \\
  2 & Tech & $[0.26, 0.26]$ & 1.07 & 0.91 & 1.20 & General \\
  3 & Sports & $[0.42, 0.42]$ & 0.83 & 0.83 & 1.09 & General \\
  4 & Tech & $[0.49, 0.49]$ & 1.18 & 0.58 & 1.06 & Tech \\
  5 & Sports & $[0.60, 0.42]$ & 0.63 & 1.04 & 0.98 & Sports \\
  6 & Tech & $[0.67, 0.40]$ & 1.21 & 0.49 & 0.93 & Tech \\
\end{tabular}
\end{center}

Initially, high uncertainty leads to exploration (General article chosen). As data accumulates, $\hat{\boldsymbol{\theta}}_t$ approaches $[0.8, 0.4]$ and the algorithm learns to match users with their preferred content.
...
for t in range(1, 101):
    ...
    r = phi @ theta_star + np.random.normal(0, 0.1)
    V += np.outer(phi, phi)
    b += r * phi

print(f"Estimated theta: {np.linalg.solve(V, b)}")  # [0.80, 0.40]
\end{lstlisting}
