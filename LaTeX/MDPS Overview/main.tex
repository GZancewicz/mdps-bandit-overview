...
\end{equation}
which is $O(K \ln T)$---logarithmic in $T$ and matching the Lai--Robbins lower bound up to constant factors for bounded rewards. Thompson Sampling also achieves asymptotically optimal regret under standard regularity conditions on the prior \cite{auer2002}. Figure~\ref{fig:regret} illustrates the qualitative separation between these growth rates.
